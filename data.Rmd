---
title: "Building dataframes from the economic calendar of Forex Factory"
output: html_document
date: "2023-02-02"
---

### Building a partial dataset excluding Forex Factory's details variables

##### Loading R packages
```{r, warning = FALSE}
library(dplyr)
library(rvest)
library(lubridate)
library(stringr)
library(zoo)
library(magrittr)
library(tidyr)
library(arrow)
```

##### Range of dates to build from
```{r}
library(arrow)

date_range = seq(as.Date("2007-01-01"), as.Date("2023-05-03"), by = "days")
day_ref = day(date_range)
month_ref = str_to_lower(month(date_range, label = TRUE))
year_ref = year(date_range)
date_ref = paste0(month_ref, day_ref, ".", year_ref)

write_feather(arrow_table(as.data.frame(date_ref)), "data/dates")
```

### create dates for updating the dataset
```{r}
library(arrow)

date_range = seq(as.Date("2023-05-04"), as.Date("2023-08-03"), by = "days")
day_ref = day(date_range)
month_ref = str_to_lower(month(date_range, label = TRUE))
year_ref = year(date_range)
date_ref = paste0(month_ref, day_ref, ".", year_ref)

write_feather(arrow_table(as.data.frame(date_ref)), "data/dates_update")
```


##### Building a dataset from the html source
```{r}
df_FF = c()

for (i in 1:length(date_ref)) {
  
  page_link = paste0("https://www.forexfactory.com/calendar?day=", date_ref[i])
  
  elements <- read_html(page_link) %>% 
    html_elements(".calendar__table") %>% 
    html_elements(".calendar__row--grey")
  
  currency <- elements %>%
    html_elements(".currency") %>%
    html_text2()
  
  event <- elements %>%
    html_elements(".event") %>%
    html_text2()
  
  forecast <- elements %>%
    html_elements(".forecast") %>%
    html_text2()
  
  observation <- elements %>%
    html_elements(".actual") %>%
    html_text2()
  
  previous_obs <- elements %>%
    html_elements(".previous") %>%
    html_text2()
  
  impact = elements %>%
    html_elements(".impact") %>%
    html_elements(".icon") %>%
    html_attr("title")
  
  event_id = elements %>%
    html_attr("data-eventid")
  
  df_day = tibble(currency,
                  event,
                  forecast,
                  observation,
                  previous_obs,
                  impact,
                  event_id) %>%
    mutate(date = as.Date(date_ref[i], format = "%b%d.%Y"))
  
  df_FF = rbind(df_FF,
                df_day)
  
  # show progress
  show(as.Date(date_ref[i], format = "%b%d.%Y"))
}

save(df_FF, file = "data/df_FF.Rdata")
```

### Building a full dataset excluding Forex Factory's details variables

##### Building a dataset from the html source
```{r, warning=FALSE}
# load("data/df_FF_full.Rdata")
df_FF_full = c()

full_html_source <- read_feather('data/arrow_source')

# date_range <- unlist(read_feather('data/dates_missing'))
# full_html_source %<>%
  # filter(date %in% date_range)

# reøove potential duplicates
full_html_source %<>%
  mutate(date_new = as.Date(date, format = "%b%d.%Y")) %>% 
  arrange(date_new) %>%
  na.omit() %>%
  distinct(date_new, .keep_all = TRUE)

# %>%
# group_by(date_new) %>%
# filter(n() > 1) %>%
# ungroup()

missing_days = c()

for (i in 1:length(date_range)) {
  
  page_i <- read_html(full_html_source$source[i])
  
  elements = page_i %>% 
    html_elements(".calendar__table") %>% 
    html_elements(".calendar__row--grey")
  
  if (length(elements) > 0) {
    
    currency <- elements %>%
      html_elements(".currency") %>%
      html_text2()
    
    event <- elements %>%
      html_elements(".event") %>%
      html_text2()
    
    forecast <- elements %>%
      html_elements(".forecast") %>%
      html_text2()
    
    observation <- elements %>%
      html_elements(".actual") %>%
      html_text2()
    
    previous_obs <- elements %>%
      html_elements(".previous") %>%
      html_text2()
    
    release_time <- elements %>%
      html_elements(".time") %>%
      html_text2()
    
    event_id = elements %>%
      html_attr("data-eventid")
    
    impact = elements %>%
      html_elements(".impact") %>%
      html_elements("span") %>%
      html_attr("title")
    
    details = elements %>%
      html_elements(".detail") %>%
      html_elements(".loaded")
    
    
    if (length(details)>0) {
      # getting the details
      df_spec = c()
      
      for (id in event_id) {
        specs = page_i %>%
          html_element(paste0("[data-eventid='", id, "'].detail")) %>%
          html_element("tbody") %>%
          html_table() %>%
          mutate(event_id = id)
        
        df_spec = rbind(df_spec, specs)
      }
      
      df_spec %<>% 
        spread(key = X1, value = X2)
      ##
      
      df_day = tibble(currency,
                      event,
                      forecast,
                      observation,
                      previous_obs,
                      release_time,
                      impact,
                      event_id) %>%
        mutate(release_time = ifelse(release_time == "", as.character(NA), release_time)) %>%
        fill(release_time) %>%
        mutate(date = full_html_source$date_new[i],
               release_time = paste0(date, " ", release_time),
               release_time = parse_date_time(release_time, "%Y %m %d %H:%M%p")) %>%
        full_join(df_spec, by = "event_id") %>%
        gather(-date, -event_id, -release_time, key = var, value = value)
      
      df_FF_full = rbind(df_FF_full,
                         df_day)
    } else {
      missing_days = c(missing_days, full_html_source$date_new[i])
      print(paste0(full_html_source$date[i], " is missing!"))
      full_html_source %<>%
        filter(date != full_html_source$date[i])
    }
    
  }
  
  # show progress
  show(full_html_source$date_new[i])
  
}

save(df_FF_full, file = "data/df_FF_full.Rdata")
```

```{r}
# write_feather(arrow_table(as.data.frame(missing_days)), "data/dates_missing")
# write_feather(arrow_table(full_html_source), "data/arrow_source")
```

