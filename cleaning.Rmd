...
econ_g = c("gdp", "ip", "manufacturing", "construction", "vehicle sales",
           ...
           "credit card spending", "pmi", "capacity utilization", "corporate profits")
...
df_FF_clean %<>%
  ...
  mutate(type = NA,
         ...
         type = ifelse(any(str_contains(event, empl)) & is.na(type), "employment", type),
         type = ifelse(any(str_contains(event, econ_g)) & is.na(type), "economic growth", type),
         type = ifelse(any(str_contains(event, confidence)) & is.na(type), "confidence", type),
         type = ifelse(any(str_contains(event, housing)) & is.na(type), "housing", type),
         type = ifelse(any(str_contains(event, infl)) & is.na(type), "inflation", type),
         type = ifelse(event == "empire state manufacturing index" & is.na(type), "economic growth", type)) %>%
  ungroup()
