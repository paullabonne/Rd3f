...
econ_g = c("gdp", "ip", "manufacturing", "vehicle sales", "construction",
...
arrow::write_parquet(df_FF_clean, "data/df_FF_clean.parquet")
```
...
df_suma = df_FF_clean %>%
  filter(geo == "USA",
         type %in% c("economic growth", "inflation", "employment", "confidence",
                      "housing")) %>%
  select(type, event) %>%
  unique() %>%
  group_by(type) %>%
...
stargazer::stargazer(df_suma, summary = FALSE, rownames=FALSE)
```
